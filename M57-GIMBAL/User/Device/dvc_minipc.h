...
{
    uint8_t header;                                        // 帧头
    float Chassis_Angular_Velocity_Yaw;                    // 底盘转动的角速度, rad/s
    float MiniPC_To_Chassis_Target_Velocity_X;             // 目标线速度 x
    float MiniPC_To_Chassis_Target_Velocity_Y;             // 目标线速度 y
    float Gimbal_Angular_Velocity_Yaw;                             // 目标角速度 w
...
public:
    ...
    // Class_MiniPC(const Class_Tricycle_Chassis chassis): chassis(chassis){}
    ...
    inline float Get_Gimbal_Angular_Velocity_Yaw();
    ...
    void Auto_aim(float x, float y, float z, float *yaw, float *pitch, float *distance, float *error);
...
protected:
    // 初始化相关常量
    // const Class_Tricycle_Chassis &chassis;
    ...
    const float bullet_v = 20.0; // 子弹速度
...
float Class_MiniPC::Get_Now_Pitch_Angle()
{
    return (Now_Angle_Pitch);
}
float Class_MiniPC::Get_Now_Yaw_Angle()
{
    return (Now_Angle_Yaw);
}
float Class_MiniPC::Get_Now_Roll_Angle()
{
 ...
 */
float Class_MiniPC::Get_Gimbal_Angular_Velocity_Yaw()
{
    return (Data_NUC_To_MCU.Gimbal_Angular_Velocity_Yaw);
}
